...
;https://www.youtube.com/feeds/videos.xml?user=channelhere
;delete C:\Program Files (x86)\VideoLAN\VLC\lua\playlist\youtube.luac and place youtube.lua
...
For $i = 1 To $iCountLines
	...
	Else
		...
		InetClose($hDownload)
		$oXML.load($sFile)
		$oParameters = $oXML.SelectNodes("/feed/entry")

		For $oParameter In $oParameters
			$oVideoId = $oParameter.SelectSingleNode("./yt:videoId").text
			...
			If (FileExists($fSave) == 0) Then
				...
				$pram = ' -vvv --qt-start-minimized "' & 'https://www.youtube.com/watch?v=' & $oVideoId & '" --qt-notification=0 --sout=#transcode{vcodec="h264",vb="1024",fps="25",vfilter=canvas{width=960,height=540},acodec="mp3",ab="12","channels=2",samplerate="32000"}:standard{access="file",dst=' & $fSave & '} vlc://quit"'
